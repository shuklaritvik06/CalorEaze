...
class RegistrationLoginTest(TestCase):
    ...
    def test_registration(self):
        ...
        self.assertEqual(DiveUser.objects.count(), 1)
        self.assertEqual(DiveUser.objects.get().username, self.username)

    def test_login(self):
        user = DiveUser.objects.create_user(username=self.username, password=self.password, email=self.email)
        data = {
            'email': self.email,
            'password': self.password
        }
